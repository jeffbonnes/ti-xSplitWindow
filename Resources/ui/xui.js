...
var applyDefaultStyles = function(newButton) {
    if (newButton.width) {
        // Do nothing, honor it...
    } else {
        if (newButton.children.length > 0) {
            // This is a view...
            newButton.width = Ti.UI.SIZE;
        } else {
            newButton.width = 60;
        }
    }
    for (x in defaultButtonStyles) {
        if (newButton[x]) {
            ...
            newButton[x] = defaultButtonStyles[x];
        }
    }
};
...
var XNavBar = function(params) {
    ...
    var titleControl, rightNavButton, leftNavButton;
    ...
    self.watch('rightNavButton', function(id, oldval, newval) {
        if (rightNavButton) {
            rightNavContainer.remove(rightNavButton);
            rightNavButton = null;
        }
        if (newval) {
            rightNavButton = newval;
            rightNavButton.right = 5;
            applyDefaultStyles(rightNavButton);
            rightNavContainer.visible = true;
            rightNavContainer.add(rightNavButton);
        } else {
            rightNavContainer.visible = false;
        }
        return newval;
    });
    ...
    self.watch('leftNavButton', function(id, oldval, newval) {
        if (leftNavButton) {
            leftNavContainer.remove(leftNavButton);
            leftNavButton = null;
        }
        if (newval) {
            leftNavButton = newval;
            leftNavButton.left = 5;
            applyDefaultStyles(leftNavButton);
            leftNavContainer.visible = true;
            leftNavContainer.add(leftNavButton);
        } else {
            leftNavContainer.visible = false;
        }
        return newval;
...
var XWindow = function(params) {
    ...
    var passThruProperties = ['top', 'bottom', 'left', 'right', 'width', 'height'];
    ...
    self.addFullScreen = function(obj) {
        self._myWindow.add(obj);
    };
